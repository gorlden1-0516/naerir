...
# 定義輸出目錄
OUTPUT_DIR = "NAER_Academic_Data_Entry_v13"
...
# ====== 頁面 1: 評鑑規則設定 (admin_evaluation_rules.html) ======
rules_content = """
<div class="row row-cols-lg-auto g-lg-4">
    <div class="col-lg-9 tw-flex-1">
        ...
        <ul class="nav nav-tabs mb-4">
            <li class="nav-item">
                <a class="nav-link active fw-bold" href="admin_evaluation_rules.html">1. 評鑑規則設定</a>
            </li>
            <li class="nav-item">
...
# ====== 頁面 2: 預警通知設定 (admin_warning_settings.html) ======
notify_content = """
<div class="row row-cols-lg-auto g-lg-4">
    <div class="col-lg-9 tw-flex-1">
        <h2 class="border-bottom tw-text-base sm:tw-text-2xl tw-font-bold overflow-hidden mb-4">
...
# 寫入 admin_evaluation_rules.html
with open(os.path.join(OUTPUT_DIR, "admin_evaluation_rules.html"), "w", encoding="utf-8") as f:
    f.write(BASE_TEMPLATE.format(
        page_title="評鑑規則設定", 
        main_content=rules_content, 
        extra_scripts=rules_script
    ))

# 寫入 admin_warning_settings.html
with open(os.path.join(OUTPUT_DIR, "admin_warning_settings.html"), "w", encoding="utf-8") as f:
    f.write(BASE_TEMPLATE.format(
        page_title="預警通知設定", 
        main_content=notify_content, 
        extra_scripts=notify_script
    ))

# ---------------------------------------------------------
# 4. 打包
# ---------------------------------------------------------
zip_filename = "NAER_Academic_Data_Entry_v12.zip"
